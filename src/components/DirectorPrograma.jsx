...
const DirectorPrograma = () => {
  const [agendas, setAgendas] = useState([]);
  const programaId = localStorage.getItem('idUser'); 
  const name = localStorage.getItem('name');

  useEffect(() => {
    const fetchAgendas = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/api/agendas/agenprograma/${programaId}`);
  ...
  const handleDownloadAndEdit = async (agendaId, decision) => {
    ...
    try {
      ...
      worksheet.getCell('G72').value = localStorage.getItem('email');
      worksheet.getCell('G73').value = localStorage.getItem('name');
      ...
      const responseUpload = await axios.post("http://localhost:8080/api/programa/update", formData, {
        headers: {
